...
景区卡片底部左侧是唯一的离线包下载入口，右侧“打开城市手册”仍只负责浏览。下载过程只在原入口显示环形进度，不打开额外页面；完成、版本过期和失败重试也在原位反馈。离线包把路线元数据、完整故事文字和当前讲述音频按版本与 SHA-256 校验后写入播放器共用缓存，可在“设置”的离线缓存项中查看并逐个清除。校验完成的路线可在断网后打开并开始，触发、播放进度和待上传状态先进入本机队列，恢复网络后按幂等键自动同步。
...
发现页先展示景点选择，再展示城市故事，不再显示“继续我的足迹”卡片。点击首页左上角“见地”可打开旅行者菜单，进入足迹、设置或退出账号。完成过的路线会保留全部已解锁线索与私密照片，再次点击路线直接进入回听；进行中的路线也会绕过首次耳机邀请。音频离开旅程页后继续播放时，发现页右侧显示可自由拖动的旋转唱片圆球，点击即可回到当前进度；开始另一景点的音频会原子停止上一景点。已解锁的绿色节点可随时回听且不改写行走进度。
