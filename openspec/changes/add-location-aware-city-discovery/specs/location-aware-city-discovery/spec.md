...
Use bounded one-shot location events to choose a useful initial city and sort only home scenic areas by their center points, without city geometry, accuracy gating, persistent discovery modes, or location tracking.

## ADDED Requirements

### Requirement: Cold discovery entry selects a backend city or Shenzhen
On cold entry to discovery, the client SHALL explain the location purpose before an undetermined operating-system permission request and SHALL perform at most one bounded real current-position attempt. It SHALL match the platform-resolved locality only against backend-provided selectable cities that contain published scenic areas. A successful match SHALL become the initial city; otherwise the client SHALL retain the existing Shenzhen default. The client MUST NOT use city coordinates, recognition radii, polygons, or distance-to-city calculations.

#### Scenario: Locality matches a selectable city with points
- **WHEN** cold-entry acquisition yields a usable position and platform locality matching a backend city with published scenic areas
- **THEN** the city selector shows that city and its home scenic-area cards are eligible for center-distance ordering

#### Scenario: Locality has no published discovery content
- **WHEN** locality is unavailable, unmatched, or matches no selectable city containing a published scenic area
- **THEN** discovery shows Shenzhen using the existing default behavior

#### Scenario: Backend adds a selectable city
- **WHEN** a newly published city and its scenic areas appear in the backend catalog and the platform locality matches its backend name
- **THEN** cold-entry selection can use it without adding a city to Flutter code

### Requirement: Refresh and city switch locate only for active-city scenic-area ordering
An explicit home refresh SHALL keep the current city and perform one bounded current-location attempt. A manual city switch SHALL apply the backend-provided selected city first and then perform one bounded current-location attempt. In both cases, a successfully acquired position SHALL be used only to sort the active city's home scenic-area cards by their centers; location MUST NOT replace the active city or reorder content inside a route.

#### Scenario: Refresh succeeds
- **WHEN** the traveler refreshes home and a current position is returned
- **THEN** the current city remains selected and its home scenic-area cards are reordered by center distance

#### Scenario: Traveler switches cities
- **WHEN** the traveler selects another backend city and location acquisition completes
- **THEN** the chosen city remains active regardless of locality and only that city's home scenic-area cards are distance-sorted

#### Scenario: Route is opened after ranking
- **WHEN** a traveler opens a distance-ranked scenic-area card
- **THEN** that scenic area retains its existing fragment/stop order and journey behavior
...
- **THEN** the client calculates and may display scenic-area-center distances without branching on its reported accuracy
